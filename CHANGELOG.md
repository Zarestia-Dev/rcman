...
- **Deterministic Setting Metadata Ordering with `IndexMap`**:
    - Replaced `std::collections::HashMap` with `indexmap::IndexMap` across `SettingsSchema::get_metadata()`, `SettingsManager::metadata()`, `SubSettingsConfig`, and `SubSettings::schema_metadata()`.
...
- Fixes an issue not skip the null values on restore if that value spefied as a null in schema.
...
- **SQLITE Storage**: Added a new SQLITE storage backend for persistent storage of settings.
...
- Fixed an issue where the primary keyring backend was not working properly.
